...
class SocketThreadToRecieve implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      in.nextLine();
      //RECIEVING PUBLIC KEY FROM THE CLIENT
      boolean publicKeyGiven = false;
      while(publicKeyGiven==false){
        String clientSentence = in.nextLine();
        System.out.println(clientSentence);
        if(clientSentence.indexOf("PUBLICKEY: ")!=-1){
          byte[] pubKey = Base64.getDecoder().decode(clientSentence.split("PUBLICKEY: ")[1]);
          
          publicKeyGiven = true;
          TCPServer.tablePublicKeys.put(username, pubKey);
          out.println("PUBLIC KEY RECIEVED: " + pubKey);

          break;
        } else {
          out.println("ERROR 104 Public Key has to be given before sending messages.");
          out.println("\n");
        }
      }
      System.out.println("Public Key is recieved. Now closing this thread.");
  ...
  public void run() {
    ...
    try {
      ...
      while (!exitApp) {
        ...
        ////RECIEVING PUBLIC KEY REQUEST////
        // if(clientSentence.indexOf("GIVE PUBLICKEY OF ")!=-1){
        //   recipientToCheck = clientSentence.split("GIVE PUBLICKEY OF ")[1];
        //   if(TCPServer.tablePublicKeys.containsKey(recipientToCheck)){
        //     byte[] publickey = TCPServer.tablePublicKeys.get(recipientToCheck);
        //     out.println(Base64.getEncoder().encodeToString(publickey));
        //   }
        //   h4 = true;
        // } else {
        ...
        if (clientSentence.indexOf("SEND ") != -1 /*&& h4*/) {
          h1 = true;
